...
with open("channels.json") as json_counties:
    ...
    for county in counties:
        ...
        live_playlist_id = playlist_items[0]['contentDetails']['relatedPlaylists']['live']
        upcoming_playlist_id = playlist_items[0]['contentDetails']['relatedPlaylists']['upcoming']
        ...
        upcoming_videos_request = youtube_service.playlistItems().list(
            part='contentDetails',
            playlistId=upcoming_playlist_id
        )
        upcoming_videos_response = upcoming_videos_request.execute()
        upcoming_videos = upcoming_videos_response['items']
        upcoming_video_ids = [video['contentDetails']['videoId'] for video in upcoming_videos]
